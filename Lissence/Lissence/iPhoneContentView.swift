...
struct iPhoneContentView: View {
    @StateObject var speechManager = SpeechManager()     // 이전에 만든 음성 인식
    @StateObject var soundDetector = SoundDetector()     // 방금 만든 소리 감지
    
    var body: some View {
        VStack(spacing: 40) {
            Text("Lissence 감지 모드 테스트")
                .font(.headline)
            
            // --- 소리 감지 섹션 ---
            VStack {
                Text("현재 상태: \(soundDetector.isDetecting ? "감지 중" : "정지됨")")
                Text("마지막 감지: \(soundDetector.lastDetectedSound)")
                    .foregroundColor(.red)
                    .bold()
                
                Button(action: {
                    if soundDetector.isDetecting {
                        soundDetector.stopDetection()
                    } else {
                        soundDetector.startDetection()
                    }
                }) {
                    Label(soundDetector.isDetecting ? "감지 중단" : "소리 감지 시작",
                          systemImage: "waveform.circle")
                        .padding()
                        .background(soundDetector.isDetecting ? Color.orange : Color.green)
                        .foregroundColor(.white)
                        .cornerRadius(10)
                }
            }
